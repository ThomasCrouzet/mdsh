// §6.9 / P2.7 - Disk synchronization via the File System Access API.
//
// Groups `openFromDisk`, `saveToDisk`, `saveActiveToDisk`, `unlinkFromDisk`
// and `refreshBrokenLinks` around `fsa.ts` + `broken-links.ts`.
//
 ...
 * System Access API (native dialog). Each file is created in the store and its
 * FSA handle is persisted in IDB (`mdsh-fs`).
 *
 * No-op if FSA is not supported or if the user cancels.
 *
...
export async function openFromDisk(deps: DiskSyncDeps): Promise<FileItem[]> {
	if (!browser || !isFSASupported()) return [];
...
/**
 * Saves a file to disk via FSA. If a handle already exists (file already
 * linked), reuses it (with a permission check). Otherwise, opens a
 * `showSaveFilePicker` dialog.
 *
 * Updates `linkedToDisk`, `brokenLink`, `dirty` on the FileItem, then calls
 * `scheduleSave` to persist the state to Dexie.
 *
 * @returns `true` if the write succeeded, `false` otherwise (FSA absent,
...
export async function saveToDisk(id: string, deps: DiskSyncDeps): Promise<boolean> {
	if (!browser || !isFSASupported()) return false;
	...
	if (handle) {
		...
		if (file.diskLastModified !== undefined) {
			try {
				...
				if (diverged) {
					const overwrite = await promptStore.confirm({
						title: t('disk.overwriteTitle'),
						message: t('disk.overwriteMessage', { name: file.name }),
...
/**
 * Unlinks a file from its disk link. Deletes the FSA handle from IDB
 * (`mdsh-fs`), then sets `linkedToDisk = false` on the FileItem and schedules
...
/**
 * Checks all `linkedToDisk` files: if the FSA handle is no longer resolvable
 * (file moved/deleted), flags `brokenLink: true` on the FileItem. If the handle
